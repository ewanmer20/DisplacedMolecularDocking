...
# alpha=0
# c=tune_c(alpha=1,target_n=8,Adjtot=Adj,nsubpsace=24)
# omega = make_omega(c,alpha)
# BIG = np.dot(np.dot(omega, laplacian(Adj)), omega)
print(BIG)
graph=nx.Graph(BIG)
print(graph.size(weight='weight'))
sol=list(nx.find_cliques(graph))
max_temp=0
max_weight=0
for i in range(len(sol)):
    cliques=graph.subgraph(sol[i])
    if max_weight<cliques.size(weight="weight"):
        max_temp=i
        max_weight=cliques.size(weight="weight")
print(sol[i])
print(max_weight)
