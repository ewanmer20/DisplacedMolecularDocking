...
class DGBS_Sampler(): 
    ...
    def __init__(self,tau=1.1,alpha=2.1, target_nsqz=None, target_ncoh=None,loss_mode=None,hbar=2, n_subspace=24,conv='real',save=False,arb_graph=False):
        """
        ...
        :param save: if True save the results in the data directory
        :param arb_graph: if True the adjacency matrix is arbitrary and not generated by the make_adj.py script
    ...
    #  Adj:is the complete adjacency matrix: not necessarily the one used for the sampling since we can take a submatrix with the dimension tuned by n_subspace!!!!
        ...
        if arb_graph==True:
            ...
            Adj,_=make_adj(tau)
            self.Adj=Adj[:n_subspace,:n_subspace]
            self.weights=make_potential_vect()[:n_subspace]
    ...
    def create_cov_mean_alt_phasenoise(self,custom_phase=[]):
        ...
        self.omega = make_omega(self.c, self.alpha,self.weights)[:self.n_subspace,:self.n_subspace]
        self.BIG=self.omega@self.Adj@self.omega
...
class PostProcessingSamples():
    ...
    def plot_success_rate_vs_OhSampler(self,niterations,plot=True):
    ...
    #This success rate is compared with the case of Oh's sampler
