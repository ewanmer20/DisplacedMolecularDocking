...
for i in range(len(loss_tot)):
    for j in range(len(x_target_ncoh)):
        ...
        np.savetxt('succ_gbs_ncoh_4uxb={:.1f}_nsqz={:.1f}_nsamples={:.1f},loss_min={:.1f},loss_max={:.1f},ncoh_min={:.1f},ncoh_max={:.1f},niteration={:.1f}.txt'.format(ncoh,nsqz,nsamples,loss_min,loss_max,ncoh_min,ncoh_max,n_iterations_local_search), np.array(succ_gbs), delimiter=',')
        np.savetxt('succ_uni_ncoh_4uxb={:.1f}_nsqz={:.1f}_nsamples={:.1f},loss_min={:.1f},loss_max={:.1f},ncoh_min={:.1f},ncoh_max={:.1f},niteration={:.1f}.txt'.format(ncoh,nsqz,nsamples,loss_min,loss_max,ncoh_min,ncoh_max,n_iterations_local_search),np.array(succ_uni),delimiter=',')
...
np.savetxt('Array_succ_gbs_nsamples_4uxb={:.1f},nsqz_min={:.1f},nsqz_max={:.1f},ncoh_min={:.1f},ncoh_max={:.1f},loss={:.1f},niteration={:.1f}.txt'.format(nsamples,loss_min,loss_max,ncoh_min,ncoh_max,nsqz,n_iterations_local_search),succ_sqzcoh_gbs,delimiter=',')
np.savetxt('Array_succ_uni_nsamples_4uxb={:.1f},nsqz_min={:.1f},nsqz_max={:.1f},ncoh_min={:.1f},ncoh_max={:.1f},loss={:.1f},niteration={:.1f}.txt'.format(nsamples,loss_min,loss_max,ncoh_min,ncoh_max,nsqz,n_iterations_local_search),succ_sqzcoh_uni,delimiter=',')
...
ax.set_title('Success rate vs loss\n and displacement with a uniform sampler after \n{:.1f} iterations and sinhr**2={:.1f}'.format(n_iterations_local_search,nsqz) )
ax.set_xlabel(r'$\langle n_{coh}\rangle$')
ax.set_ylabel(r'$Loss$')
plt.tight_layout()
plt.savefig('Success Rate vs Loss and Displacement 4uxb,nsamples={:.1f},loss_min={:.1f},loss_max={:.1f},ncoh_min={:.1f},ncoh_max={:.1f},nsqz={:.1f},niteration={:.1f}.pdf'.format(nsamples,loss_min,loss_max,ncoh_min,ncoh_max,nsqz,n_iterations_local_search),format='pdf')
fig.show()
plt.pause(10000)
