...
prob_max_clique_loop,d_alpha=probability_c(Adj, max_clique,alpha,tanhrmax,loop=True)
prob_max_clique=probability_c(Adj, max_clique,alpha,tanhrmax,loop=False)
...
for el in cliques_n:
    ...
    prob1,d_alpha = probability_c(Adj, el,alpha,tanhrmax, loop=True)
    prob2= probability_c(Adj, el,alpha,tanhrmax, loop=False)
...
ax[0].set_title('Displaced GBS histogram for the max clique and the other cliques of same size')
...
ax[1].set_title('Normal GBS histogram for the max clique and the other cliques of same size')
for a in ax.flat:
